...
// 1-Bit: flat black diamond outlined in white (with a thin white facet line standing in for
// the two-tone red/dark-red split) instead of the red/dark-red fill.
...
window.LootSprites.ruby = function(ctx, c) {
    ...
    if (onebit) {
        ...
        ctx.beginPath(); ctx.moveTo(c.x, c.y - c.radius); ctx.lineTo(c.x + c.radius, c.y); ctx.lineTo(c.x, c.y + c.radius); ctx.lineTo(c.x - c.radius, c.y); ctx.closePath(); ctx.fill();
        ctx.strokeStyle = '#ffffff'; ctx.lineWidth = Math.max(1.5, c.radius * 0.14); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(c.x, c.y); ctx.lineTo(c.x + c.radius, c.y); ctx.lineTo(c.x, c.y + c.radius); ctx.lineTo(c.x - c.radius, c.y); ctx.closePath();
        ctx.lineWidth = 1; ctx.stroke();
        ctx.globalAlpha = 0.4 + 0.6*Math.abs(Math.sin(t*2)); ctx.fillStyle = '#ffffff'; ctx.beginPath(); ctx.arc(c.x - c.radius*0.3, c.y - c.radius*0.4, 1.6, 0, Math.PI*2); ctx.fill(); ctx.globalAlpha = 1;
    } else {
